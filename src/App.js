...
function App() {   
  ...
  if(getPosts.length <1) {fetch('http://localhost:8080/getPosts').then(response => response.json()).then(response => setList(response))};
  ...
  return (
    <div className="App">
      <header className="App-header">  
        <div>
        <Router>
        <Link to="/createDatabase">  <button> Initialize Database </button> </Link>  
